...
string ResolveBrokerSymbol(const string preferredSymbol, const string baseSymbol, const bool isBuy)
{
   ...
   {
      string marketSymbol = SymbolName(i, false);
      if(marketSymbol == "")
         continue;
...
bool PlacePendingOrder(const string obj)
{
   ...
   if(!isMarketOrder && EnablePriceSanityCheck)
   {
      string priceReason = "";
   ...
   if(!validPending)
   {
      ...
      {
         Print("Pending entry invalid for ", brokerSymbol, ". Market fallback disabled; rejecting. Entry=", entry, " Bid=", bid, " Ask=", ask, " MinDistance=", minDistance);
   ...
   if(!sent || (res.retcode != TRADE_RETCODE_DONE && res.retcode != TRADE_RETCODE_PLACED))
   {
      if(res.retcode == 10033 || res.retcode == 10040)
      {
         if(!EnableMarketFallback)
         {
            Print("Pending order rejected by broker limit for ", brokerSymbol, "; market fallback disabled. retcode=", (int)res.retcode);
...
void PollBridge()
{
   ...
   if(RestrictToCurrentChartSymbol)
      url += "&mt5ChartSymbol=" + UrlEncode(_Symbol);
   string response = "";
...
int OnInit()
{
   ...
   Print("FX Bot Bridge EA build=1.17");
